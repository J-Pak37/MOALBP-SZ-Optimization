...
OUTPUT_FILENAME = f'Result_Hybrid_VNS_Soft_{TARGET_DATA_FOLDER}.csv'
...
# แทนที่ฟังก์ชัน solve_cp_subproblem_soft เดิมด้วยตัวนี้
def solve_cp_subproblem_soft(fixed_sequence, data):
   ...
   # ------------------ แทนที่บล็อก Cycle Time เดิมด้วยโค้ดนี้ ------------------
    ...
    # 4. Soft Constraints (Zoning Penalty)
    ...
    for (t1, t2) in data['zoning']:
        if t1 in fixed_sequence and t2 in fixed_sequence:
            ...
            violation = model.NewIntVar(0, PENALTY_WEIGHT, f'viol_{t1}_{t2}')
            model.Add(violation == PENALTY_WEIGHT).OnlyEnforceIf(is_same)
    ...
    # Minimize: Cost + Penalty + (ค่าน้ำหนักของสถานี เพื่อบีบให้ใช้น้อยๆ)
    STATION_WEIGHT = 50000 
    model.Minimize(total_cost + total_penalty + (max_station_used * STATION_WEIGHT))
    ...
    if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        ...
        opt_penalty = int(solver.Value(total_penalty))
        opt_stations = int(solver.Value(max_station_used))
        return opt_stations, opt_cost + opt_penalty
    else:
        # กรณีแย่สุด (แต่แทบไม่เกิดเพราะเราบังคับ Sequence ถูกต้องมาแล้ว)
        return 999, 99999999
